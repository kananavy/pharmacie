...
import { Lock, Mail, Pill, AlertCircle, Loader2, ArrowRight, ShieldCheck } from 'lucide-vue-next'
...
const handleLogin = async () => {
  ...
  error.value = ''
  try {
    const success = await auth.login({ email: form.value.email, password: form.value.password })
    if (success) {
      router.push('/')
    } else {
    ...
    <div class="w-full max-w-xl px-6 relative z-10 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div class="bg-white p-12 rounded-[3.5rem] shadow-4xl border border-emerald-50 relative overflow-hidden group">
        ...
        <form @submit.prevent="handleLogin" class="space-y-8">
          <!-- Email Input -->
          <div class="space-y-3">
            <label class="block text-[10px] font-black text-emerald-950 uppercase tracking-[0.3em] ml-2">Identifiant Professionnel</label>
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between ml-2">
              <label class="block text-[10px] font-black text-emerald-950 uppercase tracking-[0.3em]">Code d'Accès</label>
              <a href="#" class="text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:underline decoration-2 underline-offset-4">Oublié ?</a>
            </div>
            <div class="relative group/input">
          ...
          >
            ...
            <span v-if="!loading" class="transform group-hover/btn:translate-x-1 transition-transform">Se Connecter</span>
        ...
        <div class="mt-12 text-center">
          <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-relaxed">
            Logiciel de Gestion Officinale Protégé par <br> Système de Chiffrement Terminal
          </p>
        </div>
      </div>
